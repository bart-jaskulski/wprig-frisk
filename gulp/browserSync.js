...
export function serve( done ) {
	...
	if ( config.dev.browserSync.https ) {
		...
		const certPath = Object.prototype.hasOwnProperty.call( config.dev
				.browserSync, 'certPath' ) ? config.dev.browserSync.certPath :
			paths.browserSync.cert;
		const keyPath = Object.prototype.hasOwnProperty.call( config.dev
				.browserSync, 'keyPath' ) ? config.dev.browserSync.keyPath :
		...
		if ( certFound ) {
			...
			log( colors.yellow(
				`No SSL certificate found, HTTPS will ${ colors.bold( 'not' ) } be enabled`
				) );
		}
		...
		if ( keyFound ) {
			log( colors.yellow(
				`Using the SSL key ${ colors.bold( keyPath ) }` ) );
		} else {
			log( colors.yellow(
				`No SSL key found, HTTPS will ${ colors.bold( 'not' ) } be enabled`
				) );
		}
